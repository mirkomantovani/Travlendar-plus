\documentclass{article}
\usepackage{float}
\usepackage{textcomp}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{color}
\usepackage{verbatim}
\usepackage{listings}
\usepackage{underscore}
%\usepackage{showkeys}
%\usepackage{float} % here for H placement parameter
\usepackage{flafter} 
\setcounter{secnumdepth}{5}
\usepackage[bookmarks=true]{hyperref}
\author{Mirko Mantovani (893784), Matteo Marziali (893904)} 
\date{\today}
\title{Politecnico di Milano
	\\A.A. 2017\@-\@2018
	\\Software Engineering II project \\ \textbf{Travlendar+}
	\\\textbf{R}equirements \textbf{A}nalysis \\and\\ \textbf{S}pecifications \textbf{D}ocument}
	\hypersetup{pdftitle={Software Requirement Specification},    % title
	pdfauthor={Mirko Mantovani, Matteo Marziali},                     % author
	pdfsubject={RASD},                        % subject of the document
	colorlinks=true,       % false: boxed links; true: colored links
	linkcolor=blue,       % color of internal links
	citecolor=blue,       % color of links to bibliography
	filecolor=black,        % color of file links
	urlcolor=purple,        % color of external links
   }
\begin{document}
\maketitle
\begin{center}
	\includegraphics[width=5cm]{polimi-logo}
\end{center}
\clearpage
{\hypersetup{hidelinks}\tableofcontents}
\tableofcontents
\clearpage

\section{Introduction}

\subsection{Purpose}
\input{subsections/purpose}


\newpage
\subsection{Scope}
\input{subsections/scope}
\subsubsection{The world and the machine}
\input{images/worldmachine}
\clearpage
\subsection{Goals}
\input{subsections/goals}

\subsection{Definition and Acronyms}

\subsubsection{Definitions}
\input{subsections/definitions}

\subsubsection{Acronyms}
\begin{itemize}
	\item \textbf{ETA}:\@ estimated time of arrival: the time, estimated by the system, that the closest available taxi will take to get to the starting location of the ride.
	\item \textbf{API}:\@ application programming interface; it is a set of routines, protocols, and tools for building software applications on top of this one.
	
\subsection{Revision}
\input{subsections/revision}


\subsection{Actors}
\begin{itemize}
	\item \textit{Guest}: 
	\item \textit{Logged in users}:

\end{itemize}

\subsection{References}
\begin{itemize}
	\item The document with the assignment for the project
	\item The IEEE Standard for SRS 
\end{itemize}
\subsection{Document Structure}
\input{subsections/documentstructure}

\clearpage
\section{Overall description}

\subsection{Product perspective}
\input{subsections/productperspective}


\clearpage

\subsection{Product functionalities}
\input{subsections/productfunctions}

\subsection{User characteristics}
\input{subsections/usercharacteristics}


\subsection{Assumptions and dependencies}

\subsubsection{Domain Assumptions}
\input{subsections/domainassumptions}

\subsubsection{General Assumptions}
\input{subsections/assumptions}

\subsubsection{Constraints}
\input{subsections/constraints}

\clearpage
\section{Specific requirements}

\subsection{External Interface Requirements}

\subsubsection{User Interfaces}
\input{mockups/Mockups}

\clearpage
\subsubsection{Software Interfaces}
\input{subsections/softwareinterfaces}

\subsection{Non functional requirements}
\input{subsections/nonfunctionalrequirements}

\subsection{Scenarios}

\subsubsection{Scenario One}
\input{scenarios/scenario1}

\clearpage
\subsubsection{Scenario Two}
\input{scenarios/scenario2}

\clearpage
\subsection{Use Cases}
\input{sections/usecase}


\clearpage
\subsection{Activity Diagrams}
\input{sections/activitydiagrams}

\clearpage
\subsection{State Charts}
\input{sections/statecharts}

\clearpage
\subsection{Sequence Diagrams}
\input{sequencediagrams/sequencediagrams}

\clearpage
\subsection{Class Diagram}
\input{classdiagrams/classdiagram}

\clearpage
\subsection{Traceability Matrix}
\input{traceabilitytable}

\clearpage
\section{Formal Analysis using Alloy}
\subsection{Model}

\texttt{\input{alloy/alloynew.tex}}	

\clearpage
\subsection{Results}
\begin{center}
Here we provide a screenshot of the results derived from the execution of all the runs over possible predicates and checks on assertions using Alloy Analyzer.
	\begin{figure}[htp] 

\includegraphics[width=\textwidth]{alloy/material/alloyresults} 
\caption{The results of the alloy model} 
\label{fig:alloyresults} 
\end{figure} 
\end{center}
\clearpage
\subsection{Worlds Generated}
\input{sections/alloystatic}
\clearpage
\section{Appendix}

\subsection{Used software}
\input{subsections/usedsoftware}

\subsection{Effort spent}

\end{document}
