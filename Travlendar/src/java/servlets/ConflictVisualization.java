    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      for(Warning w:warnings){
          for(int i=0; i<this.extractMeetingsFromWarning(w).length;i++){
             ...
             Ms[i] = meetingFacade.find(mPK);
          }
          meetings.put(w,Ms);
          for(int j=0; j<this.extractBreaksFromWarning(w).length;j++){
              BreakPK bPK = new BreakPK(Integer.parseInt(uid),Integer.parseInt(this.extractBreaksFromWarning(w)[j]));
